...
class AppointmentControllerTest {
        ...
        void bookAppointment_Success() throws Exception {
                ...
                verify(appointmentService).bookAppointment(
                                org.mockito.ArgumentMatchers.any(AppointmentRequestDto.class),
                                eq("customer-1"));
        }
        ...
        void unauthenticatedRequest_Unauthorized() throws Exception {
                // When & Then
                mockMvc.perform(get("/appointments"))
                                .andExpect(status().isForbidden()); // Authentication configured, so 403 instead of 401
                                                                    // 401
        }
        ...
        void checkAvailability_InvalidDate_BadRequest() throws Exception {
                ...
                mockMvc.perform(get("/appointments/availability")
                                ...
                                .param("serviceType", "Oil Change")
                                .param("duration", "60"))
                                .andExpect(status().isInternalServerError()); // Type conversion error results in 500
        }
